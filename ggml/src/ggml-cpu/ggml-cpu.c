...
void ggml_compute_forward_mul_mat(
    ...
    int64_t const num_rows_per_vec_dot = type_traits_cpu[src0->type].nrows;

    
    // 把 src1 转换为 vec_dot_type
    const void * wdata = NULL;
    char * tmp = params->wdata;
        ...
        for (int64_t i12 = 0; i12 < ne12; ++i12) {
            for (int64_t i11 = 0; i11 < ne11; ++i11) {
                from_float(
                    ...
                    tmp + i13*nbw3 + i12*nbw2 + i11*nbw1,
                    ne10);
            }
        }
    }
    ...
    for (int64_t ir1 = 0; ir1 < ne1*ne2*ne3; ++ir1) {
        ...
        const int64_t i03 = i13 / (ne13 / ne03);
        ...
        const char * src1_col = (const char*)wdata +
            (i11*row_size + i12*ne11*row_size + i13*ne12*ne11*row_size);

        float * dst_col = (float*)((char*)dst->data + (i1*nb1 + i2*nb2 + i3*nb3));

        for (int64_t ir0 = 0; ir0 < ne0; ir0 += num_rows_per_vec_dot) {
            float tmp[16];
            vec_dot(ne10, tmp, 0, src0_row + ir0*nb01, 0, src1_col, 0, num_rows_per_vec_dot); 

            for (int cn = 0; cn < num_rows_per_vec_dot; ++cn) {
                dst_col[ir0 + cn*nb1/nb0] = tmp[cn*16];
            }
        }
    }
}
